...
test("ships the custom display font and viewport-contained link portal", async () => {
  ...
  assert.doesNotMatch(styles, /\.portal-atmosphere-glass|--party-glass-alpha/);
  assert.match(
    styles,
    /\.portal-atmosphere\[data-crowd="0"\]\s*\{[^}]*--party-field-saturation:\s*1\.3;/s,
  );
  assert.match(
    styles,
  ...
  assert.doesNotMatch(styles, /\.portal-house-light::after\s*\{[^}]*\bborder\s*:/s);
  ...
  for (const [, selector, declarations] of styles.matchAll(
    /([^{}]*(?::hover|:focus-visible|:active)[^{}]*)\{([^{}]*)\}/g,
  )) {
    if (!/\.portal-(?:masthead|link|footer)/.test(selector)) {
      continue;
    }
    assert.doesNotMatch(
      declarations,
